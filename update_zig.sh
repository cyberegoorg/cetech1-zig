...
function download_zig_all() {
    ...
    cp -r tmp/aarch64-macos/zig/ .
    mv zig zig_aarch64-macos
}
...
function download_zig_bin() {
    ...
    curl -L $TARBALL | tar zxv -C ./ --strip-components 1 "${FILENAME%.*.*}/zig"
    mv zig zig_${ZIG_ARCH}    
}
...
function download_zig_bin_win() {
    ...
    mv "tmp/${ZIG_ARCH}/zig/${FILENAME%.*}/zig.exe" zig_${ZIG_ARCH}.exe
}
